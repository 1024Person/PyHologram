...
class CalcHologramThead(QThread):
    _sum = pyqtSignal(int)  # 信号类型 int

    def __init__(self,fname,iter_num):
        super(CalcHologramThead,self).__init__()
        self.fname = fname
        self.iter_num = iter_num
        self.f = None
    def run(self):
        for i in CalcHologram(self.fname,self.iter_num):
            if isinstance(i,tuple):
                ...
                self._sum.emit(i)
# 相位恢复算法,也就是GS算法
def recoveryHologram(fname,iter_num):
    # 种下随机种子
    np.random.seed()
    ...
    yield imgabs * np.exp(1j*angle)

# 首先整理一下思路，这个计算全息的关键就是将物光的相位提取出来，
# 而且这个提取出来的相位还要满足，用激光去照射在这个全息图上的时候，
# 还都要满足能够把原来的图片还原出来
def CalcHologram(fname,iter_num):
    # 返回出来的是带有相位信息的物光复函数
    for i in recoveryHologram(fname, iter_num):
        yield i
